...
for drug_num in drugs_list:
    ...
    combined_ids = list(set(mut_ids) & set(drug_response_ids))
    X_m = X_mutation.loc[combined_ids]
    X_m = X_m.dropna(axis=1)
    # mutation_names = X_m.columns
    X = X_m
    coef_names = X.columns
    X.to_csv('./data_outputs/Lasso_only_mut/X_' + inhibitors_list[drug_num] + '.csv')
    drug_response = drug_response.loc[combined_ids]
    Y = drug_response.sort_index()
    Y.to_csv('./data_outputs/Lasso_only_mut/Y_' + inhibitors_list[drug_num] + '.csv')
    ...
    file = open("./regression_outputs/Lasso_only_mut/" + inhibitors_list[drug_num] + ".txt", 'w+')
    ...
    print(list(zip(lasso_01.coef_, coef_names)), file=file)
    ...
    print(list(zip(lasso_001.coef_, coef_names)), file=file)
    file.close()
